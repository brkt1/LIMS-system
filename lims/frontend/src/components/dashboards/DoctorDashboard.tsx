import { Calendar, CheckCircle, ClipboardList, Clock, FileText, Stethoscope, Users } from 'lucide-react';
    ...
    <BaseDashboard>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div className="card">
          ...
          <div className="space-y-4">
            {[
              ...
              { time: '10:30 AM', patient: 'Sarah Johnson', type: 'New Patient', status: 'Confirmed' },
              { time: '02:00 PM', patient: 'Mike Davis', type: 'Consultation', status: 'Pending' },
              { time: '03:30 PM', patient: 'Lisa Wilson', type: 'Test Review', status: 'Confirmed' },
              { time: '04:45 PM', patient: 'Robert Brown', type: 'Follow-up', status: 'Confirmed' }
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            {[
              { patient: 'John Smith', test: 'Blood Panel', result: 'Normal', priority: 'Low' },
              { patient: 'Sarah Johnson', test: 'X-Ray Chest', result: 'Abnormal', priority: 'High' },
              { patient: 'Mike Davis', test: 'Urine Analysis', result: 'Normal', priority: 'Low' },
              { patient: 'Lisa Wilson', test: 'MRI Brain', result: 'Pending', priority: 'Medium' }
            ].map((test, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center space-x-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    test.priority === 'High' ? 'bg-red-100' : 
                    test.priority === 'Medium' ? 'bg-yellow-100' : 'bg-green-100'
                  }`}>
                ...
                <div className="text-right">
                  <span className={`text-xs font-semibold px-2 py-1 rounded-full ${
                    ...
                    test.result === 'Abnormal' ? 'bg-red-100 text-red-800' :
                    'bg-yellow-100 text-yellow-800'
                  }`}>
                    {test.result}
                  </span>
